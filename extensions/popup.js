...
(function() {
  ...
  function updateUI(status) {
    ...
    if (status.lastSync) {
      ...
      else if (sec < 300) stTime.textContent = Math.floor(sec / 60) + "분 전";
      else stTime.textContent = "동기화됨";
    } else {
      stTime.textContent = "—";
    }
    ...
    if (currentState === "OPENING_TABS") {
      ...
      progress.innerHTML = '<div class="step"><span class="icon">⏳</span> 연결 중...</div>';
    } else if (currentState === "WAITING_DATA") {
      progress.style.display = "block";
      progress.innerHTML = '<div class="step"><span class="icon">📊</span> 로딩 중...</div>';
    } else {
