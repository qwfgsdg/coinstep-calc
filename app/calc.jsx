...
// ── 강청가 표시 헬퍼 ──
// noLiq = 해당 코인 가격이 어디로 가든 청산 조건에 도달하지 않는 상태.
// 0으로 표시하면 "청산 임박"으로 정반대 오독되므로 반드시 구분해서 표기한다.
const liqLabel = (d) =>
  !d ? "—"
    : d.liquidated ? "청산 구간"
    : d.noLiq ? "청산 없음"
    : d.liq != null ? `$${fmt(d.liq, d.liq > 100 ? 2 : 4)}` : "—";
const distLabel = (d) =>
  !d ? "—" : d.liquidated ? "0%" : d.noLiq ? "∞" : d.dist == null ? "—" : `${fmt(Math.abs(d.dist))}%`;
const warnBox = (c) => ({
  padding: "6px 10px", borderRadius: 6, background: c + "10",
  border: `1px solid ${c}33`, fontSize: 10, color: c, fontFamily: "'DM Sans'",
});
const liqImproved = (before, after) => {
  if (!after || after.liquidated) return false;   // 이미 청산 구간 = 최악
  if (before && before.liquidated) return true;   // 청산 구간에서 벗어남
  if (after.noLiq) return true;                   // 청산이 사라짐 = 최선
  if (before && before.noLiq) return false;       // 없던 청산이 생김 = 악화
  return !!(before && before.dist != null && after.dist != null &&
...
export default function SimV4() {
  ...
  // ── 입출금 시뮬레이션 ──
  // 금액 / 목표강청가 / 목표거리% 는 서로 연동되지만, 상태로 들고 있는 건 "사용자가 마지막에 친 칸" 하나뿐이다.
  // 셋을 모두 state로 두고 서로 동기화하면 되먹임 루프가 생기므로 나머지 둘은 계산해서 표시만 한다.
  const [flowMode, setFlowMode] = useState("deposit");   // "deposit" | "withdraw"
  const [flowSrc, setFlowSrc] = useState("amount");      // "amount" | "liq" | "dist"
  const [flowAmount, setFlowAmount] = useState("");      // 입력 금액 (항상 양수로 취급)
  const [flowTargetLiq, setFlowTargetLiq] = useState("");
  ...
  const calc = useMemo(() => {
    ...
    // denom(= dP/dD 의 역수)까지 함께 돌려주는 상세 버전.
    // 입출금 시뮬의 민감도·역산이 denom을 필요로 한다.
    const solveLiqDetail = (targetCoin, posArr, mmr, walletBal) => {
      if (!mmr || mmr <= 0) return null;
      const w = walletBal != null ? walletBal : wb;
      ...
      const hedged = targetPos.some(p => p.sign > 0) && targetPos.some(p => p.sign < 0);

      // 현재가 기준 건전성. health(P) = equity(P) − mmr×notional(P) 이고 P에 대해 1차이므로
      // health(현재가) ≤ 0 이면 가격이 어디에 있든 이미 유지증거금을 밑돈 상태다.
      // 이 검사가 없으면 잔고가 크게 마이너스일 때 나오는 음수 해를
      // "청산 없음"(가장 안전)으로 정반대 해석하게 된다.
      const priced = posArr.every(p => p.pcp > 0);
      let liquidated = false;
      if (priced) {
        const pnlNow = posArr.reduce((a, p) => a + p.sign * (p.pcp - p.ep) * p.qty, 0);
        const notionalNow = posArr.reduce((a, p) => a + p.qty * p.pcp, 0);
        liquidated = (w + pnlNow) - mmr * notionalNow <= 0;
      }
      if (liquidated) return { liq: null, denom, noLiq: false, liquidated: true, hedged };

      // denom ≈ 0 : 가격이 어디로 가도 청산 조건에 도달하지 않음
      if (Math.abs(denom) < 1e-12) return { liq: null, denom, noLiq: true, liquidated: false, hedged };
      const liq = numer / denom;
      // 건전한 상태에서의 음수 해 = 이 코인 가격만으로는 청산 도달 불가 (0으로 뭉개면 정반대로 읽힌다)
      if (!isFinite(liq) || liq <= 0) return { liq: null, denom, noLiq: true, liquidated: false, hedged };
      return { liq, denom, noLiq: false, liquidated: false, hedged };
    };

    // 기존 호출부 호환 래퍼 — 반환 계약(숫자 | 0 | null)을 그대로 유지한다.
    const solveLiqForCoin = (targetCoin, posArr, mmr, walletBal) => {
      const d = solveLiqDetail(targetCoin, posArr, mmr, walletBal);
      if (!d) return null;
      return d.noLiq ? 0 : d.liq;
    };
    ...
    const calcLiqPerCoin = (posArr, mmr, walletBal) => {
      ...
      coins.forEach(coin => {
        ...
        const d = solveLiqDetail(coin, posArr, mmr, walletBal);
        if (!d) return;
        // 청산 불가도 항목으로 남긴다 — 빼버리면 before/after 표에서 "변화 없음"으로 오독된다.
        result[coin] = {
          liq: d.liq,
          dist: d.liq != null && coinCp > 0 ? ((coinCp - d.liq) / coinCp) * 100 : null,
          cp: coinCp,
          noLiq: !!d.noLiq,
    ...
    // ── 입출금 시뮬레이션 ──
    // 포지션은 전혀 건드리지 않고 지갑잔고 하나만 바꾼다.
    //   P_liq(w + D) = P_liq(w) − D / denom   (denom 은 D 와 무관 → 완전 선형)
    let flowResult = null;
    if (mmRate && parsed.length > 0) {
      const refCoins = Object.keys(baseLiqPerCoin);

      // 기준 코인: 사용자 지정 → 없으면 안전거리 절대값이 가장 작은(=가장 위험한) 코인
      let refCoin = flowRefCoin && refCoins.includes(flowRefCoin) ? flowRefCoin : "";
      if (!refCoin) {
        let best = "";
        refCoins.forEach(c => {
          const d = baseLiqPerCoin[c];
          if (!d || d.noLiq || d.dist == null) return;
          if (!best || Math.abs(d.dist) < Math.abs(baseLiqPerCoin[best].dist)) best = c;
        });
        refCoin = best || refCoins[0] || "";
      }

      const refData = refCoin ? baseLiqPerCoin[refCoin] : null;
      const refDenom = refData ? refData.denom : null;
      const refCp = refCoin ? n(coinPrices[refCoin] || "") : 0;
      const denomOk = refDenom != null && Math.abs(refDenom) > 1e-12;

      // 민감도: 1 USDT 입금당 강청가 이동폭
      const sensitivity = denomOk ? -1 / refDenom : null;
      // 역산 가능 여부 — 현재 강청가가 없으면 "목표 강청가"의 기준점이 없다
      const inverseOk = denomOk && refData != null && !refData.noLiq && refData.liq != null;

      const sign = flowMode === "withdraw" ? -1 : 1;
      let D = 0;
      let targetLiqUsed = null;
      let unreachable = false;

      if (flowSrc === "amount") {
        D = sign * Math.abs(n(flowAmount));
      } else if (inverseOk) {
        if (flowSrc === "liq") {
          const p = n(flowTargetLiq);
          if (p > 0) targetLiqUsed = p; else unreachable = n(flowTargetLiq) !== 0;
        } else if (flowSrc === "dist") {
          const r = n(flowTargetDist);
          if (r !== 0 && refCp > 0) {
            // 방향은 사용자가 고르지 않고 순포지션 방향(denom 부호)에서 자동 판정한다.
            targetLiqUsed = refDenom > 0
              ? refCp * (1 - Math.abs(r) / 100)   // 순롱 → 강청가는 현재가 아래
              : refCp * (1 + Math.abs(r) / 100);  // 순숏 → 강청가는 현재가 위
            if (targetLiqUsed <= 0) { targetLiqUsed = null; unreachable = true; }
          }
        }
        if (targetLiqUsed != null) D = -refDenom * (targetLiqUsed - refData.liq);
      }

      const active = isFinite(D) && Math.abs(D) > 1e-9;
      const simWallet = wb + D;
      const afterLiqPerCoin = active ? calcLiqPerCoin(parsed, mmRate, simWallet) : null;
      const afterRef = afterLiqPerCoin ? afterLiqPerCoin[refCoin] : null;

      // 역산 결과가 반대 방향이면 지금 탭에서는 처리할 수 없다
      const wrongDirection = active && flowSrc !== "amount" &&
        ((flowMode === "deposit" && D < 0) || (flowMode === "withdraw" && D > 0));

      flowResult = {
        refCoin, refCoins, refData, refCp, sensitivity, inverseOk, unreachable, wrongDirection,
        mode: flowMode, src: flowSrc, active,
        D, amountAbs: Math.abs(D),
        simWallet, newFreeMargin: freeMargin + D,
        baseLiqPerCoin, afterLiqPerCoin, afterRef,
        // 사용자가 입력하지 않은 나머지 두 칸에 채워 넣을 값
        derivedLiq: afterRef && !afterRef.noLiq ? afterRef.liq : null,
        derivedDist: afterRef && afterRef.dist != null ? Math.abs(afterRef.dist) : null,
        derivedNoLiq: !!(afterRef && afterRef.noLiq),
        // 경고
        overFreeMargin: flowMode === "withdraw" && active && Math.abs(D) > Math.max(freeMargin, 0),
        overFreeBy: Math.abs(D) - Math.max(freeMargin, 0),
        overWallet: flowMode === "withdraw" && active && Math.abs(D) > wb,
        overWalletBy: Math.abs(D) - wb,
      };
    }
  ...
  const flowInpStyle = (isSrc, disabled) => ({
    width: "100%", padding: "6px 8px", background: disabled ? "var(--bg-panel)" : "var(--bg-input)",
    border: `1px solid ${isSrc ? "#0ea5e9" : "var(--border)"}`, borderRadius: 6,
    color: disabled ? "var(--text-dim)" : "var(--text-bright)",
    fontSize: 12, fontFamily: "'IBM Plex Mono'", outline: "none",
  });
  // 3칸 연동 — 방금 친 칸만 source 로 남기고 나머지는 파생값으로 표시한다
  const onFlowAmount = (v) => { setFlowSrc("amount"); setFlowAmount(v); setFlowTargetLiq(""); setFlowTargetDist(""); };
  const onFlowLiq = (v) => { setFlowSrc("liq"); setFlowTargetLiq(v); setFlowAmount(""); setFlowTargetDist(""); };
  const onFlowDist = (v) => { setFlowSrc("dist"); setFlowTargetDist(v); setFlowAmount(""); setFlowTargetLiq(""); };
      ...
      <div style={S.wrap}>
        ...
        {calc && hasAnyPrice && (
          <>
            ...
            {/* ── 입출금 시뮬레이션 ── */}
            {positions.length > 0 && !fr && (
              <div style={{ marginTop: 10, padding: "10px 14px", borderRadius: 10, background: "var(--bg-card)", border: "1px solid var(--border)", fontSize: 11, color: "var(--text-dim)", fontFamily: "'DM Sans'" }}>
                입출금 시뮬레이션 — 거래소 강제청산가가 동기화되어야 계산됩니다
              </div>
            )}
            {fr && (
              <div style={{ marginTop: 10, padding: 14, borderRadius: 10, background: "var(--bg-card)", border: `1px solid ${fr.mode === "withdraw" ? "#f8717122" : "#34d39922"}` }}>
                {/* 헤더: 제목 + 입금/출금 토글 + 기준코인 */}
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                  <div style={{ width: 3, height: 14, background: fr.mode === "withdraw" ? "#f87171" : "#34d399", borderRadius: 2 }} />
                  <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", color: fr.mode === "withdraw" ? "#f87171" : "#34d399", fontFamily: "'DM Sans'" }}>
                    입출금 시뮬레이션
                  </span>
                  <div style={{ display: "flex", gap: 4, marginLeft: 4 }}>
                    {[{ id: "deposit", label: "입금", c: "#34d399" }, { id: "withdraw", label: "출금", c: "#f87171" }].map(m => (
                      <button key={m.id} onClick={() => setFlowMode(m.id)} style={{
                        padding: "4px 12px", fontSize: 11, fontWeight: 700, borderRadius: 6, cursor: "pointer", fontFamily: "'DM Sans'",
                        border: `1px solid ${fr.mode === m.id ? m.c + "66" : "var(--border)"}`,
                        background: fr.mode === m.id ? m.c + "15" : "transparent",
                        color: fr.mode === m.id ? m.c : "var(--text-dim)",
                      }}>{m.label}</button>
                    ))}
                  </div>
                  <div style={{ flex: 1 }} />
                  {fr.refCoins.length > 1 && (
                    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                      <span style={{ fontSize: 9, color: "var(--text-dim)", fontFamily: "'DM Sans'" }}>기준</span>
                      {fr.refCoins.map(c => (
                        <button key={c} onClick={() => setFlowRefCoin(flowRefCoin === c ? "" : c)} style={{
                          padding: "3px 8px", fontSize: 10, fontWeight: 700, borderRadius: 5, cursor: "pointer", fontFamily: "'IBM Plex Mono'",
                          border: `1px solid ${fr.refCoin === c ? "#0ea5e966" : "var(--border)"}`,
                          background: fr.refCoin === c ? "#0ea5e915" : "transparent",
                          color: fr.refCoin === c ? "#0ea5e9" : "var(--text-dim)",
                        }}>{c}</button>
                      ))}
                    </div>
                  )}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  <div>
                    <div style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 4, fontFamily: "'DM Sans'" }}>
                      {fr.mode === "withdraw" ? "출금액" : "입금액"} (USDT)
                    </div>
                    <input type="number" placeholder="0"
                      value={fr.src === "amount" ? flowAmount : (fr.active ? String(Math.round(fr.amountAbs * 100) / 100) : "")}
                      onChange={e => onFlowAmount(e.target.value)}
                      style={flowInpStyle(fr.src === "amount", false)} />
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 4, fontFamily: "'DM Sans'" }}>
                      목표 강청가 {fr.refCoin && `(${fr.refCoin})`}
                    </div>
                    <input type="number" placeholder={fr.inverseOk ? "0" : "역산 불가"} disabled={!fr.inverseOk}
                      value={fr.src === "liq" ? flowTargetLiq : (fr.derivedLiq != null ? String(Math.round(fr.derivedLiq * 10000) / 10000) : "")}
                      onChange={e => onFlowLiq(e.target.value)}
                      style={flowInpStyle(fr.src === "liq", !fr.inverseOk)} />
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 4, fontFamily: "'DM Sans'" }}>
                      목표 안전거리 (%)
                    ...
                    <input type="number" placeholder={fr.inverseOk ? "0" : "역산 불가"} disabled={!fr.inverseOk}
                      value={fr.src === "dist" ? flowTargetDist : (fr.derivedDist != null ? String(Math.round(fr.derivedDist * 10) / 10) : "")}
                      onChange={e => onFlowDist(e.target.value)}
                      style={flowInpStyle(fr.src === "dist", !fr.inverseOk)} />
                  </div>
                </div>
                ...
                <div style={{ display: "flex", gap: 4, marginTop: 6, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  {fr.sensitivity != null && fr.refCoin && (
                    <span style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "'DM Sans'" }}>
                      {fr.refCoin} 1,000 USDT 당 강청가{" "}
                      <span style={{ color: "#0ea5e9", fontWeight: 700, fontFamily: "'IBM Plex Mono'" }}>
                        {fmtS(fr.sensitivity * 1000 * (fr.mode === "withdraw" ? -1 : 1), 2)}
                      </span>
                    </span>
                  )}
                </div>
                ...
                {/* 경고 */}
                {(fr.overWallet || fr.overFreeMargin || fr.wrongDirection || fr.unreachable || !fr.inverseOk) && (
                  <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
                    {fr.overWallet && (
                      <div style={warnBox("#f87171")}>지갑잔고 초과 — {fmt(fr.overWalletBy)} USDT 모자랍니다</div>
                    )}
                    {fr.overFreeMargin && !fr.overWallet && (
                      <div style={warnBox("#f59e0b")}>여유마진 초과 {fmt(fr.overFreeBy)} USDT — 실제 거래소에서는 출금이 거절될 수 있습니다</div>
                    )}
                    {fr.wrongDirection && (
                      <div style={warnBox("#f59e0b")}>
                        이 목표는 {fr.mode === "deposit" ? "출금" : "입금"}이 필요합니다 — {fr.mode === "deposit" ? "출금" : "입금"} 버튼으로 전환하세요
                      </div>
                    )}
                    {fr.unreachable && (
                      <div style={warnBox("#f59e0b")}>도달할 수 없는 목표입니다</div>
                    )}
                    {!fr.inverseOk && (
                      <div style={warnBox("#f59e0b")}>
                        {fr.refData?.liquidated
                          ? `${fr.refCoin} 은(는) 이미 유지증거금을 밑돈 청산 구간이라 목표 역산의 기준점이 없습니다 — 금액으로 입력하세요`
                          : `${fr.refCoin} 은(는) 현재 청산이 발생하지 않는 상태라 목표 역산을 할 수 없습니다 — 금액으로 입력하세요`}
                      </div>
                    )}
                  </div>
                )}
                ...
                {/* 결과 */}
                {fr.active && fr.afterLiqPerCoin && (
                  <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--border)" }}>
                    <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(fr.refCoins.length, 4)}, 1fr)`, gap: 6 }}>
                      {fr.refCoins.map(coin => {
                        const before = fr.baseLiqPerCoin[coin];
                        const after = fr.afterLiqPerCoin[coin];
                        if (!before || !after) return null;
                        const improved = liqImproved(before, after);
                        const delta = before.liq != null && after.liq != null ? after.liq - before.liq : null;
                        return (
                          <div key={coin} style={{ padding: 8, borderRadius: 6, background: "var(--bg-panel)", border: `1px solid ${improved ? "#34d39933" : "#f8717133"}`, textAlign: "center" }}>
                            <div style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 600, fontFamily: "'DM Sans'" }}>{coin}</div>
                            <div style={{ fontSize: 13, fontWeight: 700, color: improved ? "#34d399" : "#f87171", fontFamily: "'DM Sans'", marginTop: 2 }}>
                              {delta != null ? fmtS(delta, Math.abs(delta) > 100 ? 1 : 2) : liqLabel(after)}
                            </div>
                            <div style={{ fontSize: 9, color: "var(--text-dim)", marginTop: 2, fontFamily: "'DM Sans'" }}>
                              {liqLabel(before)} → <span style={{ color: improved ? "#34d399" : "#f87171", fontWeight: 600 }}>{liqLabel(after)}</span>
                            </div>
                            <div style={{ fontSize: 9, color: "var(--text-dim)", marginTop: 1, fontFamily: "'DM Sans'" }}>
                              여유 {distLabel(before)} → <span style={{ fontWeight: 600 }}>{after.noLiq ? "∞" : distLabel(after)}</span>
                            </div>
                          </div>
                        );
                      })}
                    </div>
                    <div style={{ display: "flex", gap: 16, marginTop: 8, fontSize: 11, fontFamily: "'DM Sans'", flexWrap: "wrap" }}>
                      <span style={{ color: "var(--text-muted)" }}>
                        지갑 <span style={{ color: "var(--text-bright)", fontWeight: 600, fontFamily: "'IBM Plex Mono'" }}>{fmt(calc.wb, 0)} → {fmt(fr.simWallet, 0)}</span>
                      </span>
                      <span style={{ color: "var(--text-muted)" }}>
                        여유마진 <span style={{ color: fr.newFreeMargin > 0 ? "#34d399" : "#f87171", fontWeight: 600, fontFamily: "'IBM Plex Mono'" }}>{fmt(calc.freeMargin, 0)} → {fmt(fr.newFreeMargin, 0)}</span>
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(Object.keys(calc.baseLiqPerCoin).length, 4)}, 1fr)`, gap: 6 }}>
                  {Object.entries(calc.baseLiqPerCoin).map(([coin, data]) => (
                    <div key={coin} style={{ padding: 10, borderRadius: 8, background: "var(--bg-panel)", border: "1px solid var(--border)", textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: data.liq != null ? 15 : 12, fontWeight: 700, color: data.noLiq ? "#34d399" : "#f87171", fontFamily: "'DM Sans'", margin: "4px 0 2px" }}>
                        {liqLabel(data)}
                      </div>
                      <div style={{ fontSize: 10, color: data.noLiq ? "#34d399" : data.liquidated ? "#f87171" : data.dist > 0 ? "#34d399" : "#f87171" }}>
                        여유 {distLabel(data)}
                      </div>
                    </div>
                  ))}
                            ...
                            return (
                              <div key={coin} style={{ padding: 8, borderRadius: 6, background: "var(--bg-card)", border: `1px solid ${improved ? "#34d39933" : "#f8717133"}`, textAlign: "center" }}>
                                ...
                                <div style={{ fontSize: 9, color: "var(--text-dim)", marginTop: 2 }}>
                                  {before.noLiq ? "∞" : distLabel(before)} → <span style={{ color: improved ? "#34d399" : "#f87171", fontWeight: 600 }}>{after.noLiq ? "∞" : distLabel(after)}</span>
                                </div>
                              </div>
                            );
                          })}
                          ...
                          {Object.entries(calc.reduceResult.baseLiqPerCoin).map(([coin, before]) => {
                            ...
                            return (
                              <div key={coin} style={{ padding: 8, borderRadius: 6, background: "var(--bg-card)", border: `1px solid ${improved ? "#34d39933" : after ? "#f8717133" : "var(--border)"}`, textAlign: "center" }}>
                                <div style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 600 }}>{coin}</div>
                                <div style={{ fontSize: 12, fontWeight: 700, color: improved ? "#34d399" : "#f87171", fontFamily: "'DM Sans'", marginTop: 2 }}>
                                  {after ? liqLabel(after) : "—"}
                                </div>
                                <div style={{ fontSize: 9, color: "var(--text-dim)", marginTop: 2 }}>
                                  {before.noLiq ? "∞" : distLabel(before)} → <span style={{ color: improved ? "#34d399" : "#f87171", fontWeight: 600 }}>{after ? (after.noLiq ? "∞" : distLabel(after)) : "—"}</span>
                                </div>
                              </div>
                            );
                          })}
              ...
              {Object.entries(baseLiqPerCoin).map(([coin, before]) => {
                ...
                return (
                  <tr key={coin}>
                    ...
                    <TD>{liqLabel(before)}</TD>
                    <TD c={improved ? "#34d399" : "#f87171"}>
                      {after ? liqLabel(after) : "—"}
                    </TD>
                    <TD c={improved ? "#34d399" : "#f87171"}>
                      {before.noLiq ? "∞" : distLabel(before)}
                      {" → "}
                      <span style={{ fontWeight: 600 }}>
                        {after ? (after.noLiq ? "∞" : distLabel(after)) : "—"}
                      </span>
                      {improved ? " ▼" : after ? " ▲" : ""}
                    </TD>
                  </tr>
